...
export function slugifyFilterName(filterName: string): string {
  // Polish characters that don't decompose via NFD normalization
  ...
  return filterName
    .toLowerCase()
    .replace(/[łŁ]/g, (char) => polishCharMap[char] || char) // Handle Polish ł/Ł first
 ...
 * e.g., ?impedancja-min=4&impedancja-max=12 → [{ filterName: "Impedancja", minValue: 4, maxValue: 12 }]
 *
...
export function parseRangeFilters(
  ...
  filterDefinitions
    ...
    .forEach((filter) => {
      ...
      const minParam = searchParams.get(`${slug}-min`);
      const maxParam = searchParams.get(`${slug}-max`);
