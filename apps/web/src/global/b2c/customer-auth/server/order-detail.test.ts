...
function setupSupabaseMock(args: {
  ...
  const cancellationRequestMaybeSingleMock = vi.fn().mockResolvedValue({
    data: args.cancellationRequest ?? null,
    error: null,
  });
  const cancellationRequestLimitMock = vi.fn(() => ({
    maybeSingle: cancellationRequestMaybeSingleMock,
  }));
  const cancellationRequestOrderMock = vi.fn(() => ({
    limit: cancellationRequestLimitMock,
  }));
...
describe('loadCustomerOrderForPanel', () => {
  ...
  it('maps rejected cancellation requests and disables repeat cancellation', async () => {
    setupSupabaseMock({
      cancellationRequest: {
        admin_note: 'Zamówienie jest już w pakowaniu.',
        customer_message: 'Proszę anulować.',
        id: 'request-1',
        reason: 'changed_mind',
    ...
    expect(result.order.cancellationRequest).toEqual({
      ...
      customerMessage: 'Proszę anulować.',
      reason: 'changed_mind',
      requestedAt: '2026-04-27T08:00:00.000Z',
      resolvedAt: '2026-04-28T08:00:00.000Z',
      status: 'rejected',
    });
    expect(result.order.actions.canCancel).toBe(false);
