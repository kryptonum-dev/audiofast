 ...
 */
export function getParentCategorySanityId(
  deviceTypeId: number
): string | null {
  const mapping = PARENT_CATEGORY_MAPPINGS.find((m) => m.sqlId === deviceTypeId);
...
export function generateSEODescription(
  ...
  const specificDescriptions: Record<string, string> = {
    ...
    Półki: 'Półki i stojaki audio dla optymalnej organizacji sprzętu. Funkcjonalność i estetyka.',
    Stoliki:
...
export function validateSubCategory(
  ...
  if (
    ...
    subCategory.seo.description.length < 80
  ) {
    errors.push({
      field: 'seo.description',
