 ...
 */
export function transformDealerToStore(
  dealer: DealerRecord
): { document: SanityStoreDocument; warnings: ValidationError[] } {
...
export function validateStoreDocument(
  ...
  if (!document.address.postalCode || !/^\d{2}-\d{3}$/.test(document.address.postalCode)) {
    errors.push({
      dealerId,
