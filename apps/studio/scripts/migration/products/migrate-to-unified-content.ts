...
async function clearProductUnifiedContent(
  ...
  if (dryRun) {
    console.log(
      '   🔍 DRY RUN - Would clear details.productDetailContent from:',
    );
  ...
  for (const product of products) {
    await client
      .patch(product._id)
      .unset(['details.productDetailContent'])
      .commit();
  }

  console.log(
    `   ✅ Cleared details.productDetailContent from ${products.length} products.\n`,
  );
}
...
async function main() {
  ...
  console.log(
    '║     details.content → details.productDetailContent               ║',
  );
  ...
  if (forceOverwrite) {
    console.log(
      `   Force: Will overwrite existing details.productDetailContent`,
    );
  }
  if (clearFirst) {
  ...
  for (const product of products) {
    const brandInfo = (product as any).brandName
  ...
  if (dryRun) {
    ...
    console.log(
      '💡 Use --clear to clear existing details.productDetailContent first',
    );
    console.log(
      '💡 Use --force to overwrite existing details.productDetailContent',
    );
  }
}
