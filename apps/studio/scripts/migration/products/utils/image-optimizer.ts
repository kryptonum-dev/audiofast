  ...
  return new Promise((resolve) => {
    const handleResponse = (response: https.IncomingMessage) => {
      ...
      if (response.statusCode === 301 || response.statusCode === 302) {
        ...
        https.get(redirectUrl, { agent: insecureAgent }, handleResponse).on('error', () => resolve(null));
        return;
      }

      if (response.statusCode !== 200) {
        console.warn(`   ⚠️  Failed to fetch image (HTTP ${response.statusCode}): ${url}`);
        resolve(null);
        return;
      }
    ...
    https.get(url, { agent: insecureAgent }, handleResponse).on('error', () => resolve(null));
  });
}
...
async function optimizeImage(
  ...
  skipUpscaling = false
): Promise<{ buffer: Buffer; wasUpscaled: boolean; originalWidth: number; targetWidth: number }> {
...
export async function processAndUploadImage(
  ...
  const { imageType = 'gallery', skipOptimization = false, skipUpscaling } = options;
  ...
  try {
    ...
    for (const url of urlsToTry) {
      ...
      if (originalBuffer && originalBuffer.length > 0) {
        successUrl = url;
        if (url !== sourceUrl) {
          console.log(`   📍 Found at alternative URL: ${url.split('/').pop()}`);
        }
        break;
      }
    }
    ...
    if (skipOptimization) {
      ...
      // 2. Optimize image
      const result = await optimizeImage(originalBuffer, config, shouldSkipUpscaling);
...
export async function processImageWithFallback(
  ...
  if (!result) {
    console.log(`   🔄 Retrying without optimization: ${sourceUrl.split('/').pop()}`);
