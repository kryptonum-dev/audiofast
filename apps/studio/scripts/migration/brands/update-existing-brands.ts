...
const BRAND_MAPPING: Record<string, { sanityId: string; csvId: string; name: string }> = {
  'weiss': {
...
function findBrandInCSV(rows: CSVRow[], csvId: string): BrandData | null {
  ...
  return {
    ...
    bannerImageFilename: brandRows.find(r => r.BannerImageFilename)?.BannerImageFilename || null,
  };
}
...
async function main() {
  ...
  console.log('╔════════════════════════════════════════════════════════════════╗');
  console.log('║         UPDATE EXISTING BRANDS                                 ║');
  console.log('╚════════════════════════════════════════════════════════════════╝');
  ...
  for (let i = 0; i < brandKeys.length; i++) {
    ...
    console.log(`  Hero Description: ${csvData.heroDescription ? 'Yes' : 'No'}`);
    ...
    try {
      ...
      const seoDescription = generateSeoDescription(mapping.name, csvData.heroDescription);
      ...
      if (csvData.bannerImageFilename && client && !dryRun) {
        ...
        const filename = csvData.bannerImageFilename.split('/').pop() || 'banner.jpg';
        bannerImageRef = await uploadImageToSanity(client, bannerUrl, filename);
      } else if (csvData.bannerImageFilename) {
        console.log(`  Banner: ${csvData.bannerImageFilename} (dry-run, not uploading)`);
      }
      ...
      const patchData: Record<string, any> = {
        description: heroDescription.length > 0 
          ? heroDescription 
          : textToPortableText(`Odkryj produkty marki ${mapping.name} w ofercie Audiofast.`),
        heroImage: {
          _type: 'image',
          asset: {
            _type: 'reference',
