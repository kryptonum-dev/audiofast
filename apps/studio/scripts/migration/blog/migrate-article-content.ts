...
function loadProductSlugMap(): Map<string, string> {
  ...
  try {
    ...
    console.warn(
      `   ⚠️  Could not load product slugs CSV: ${err instanceof Error ? err.message : err}`,
    );
    productSlugMap = new Map<string, string>();
    return productSlugMap;
  }
...
function loadSiteTreeMap(): Map<string, string> {
  ...
  try {
    ...
    console.warn(
      `   ⚠️  Could not load sitetree slugs CSV: ${err instanceof Error ? err.message : err}`,
    );
    sitetreeSlugMap = new Map<string, string>();
    return sitetreeSlugMap;
  }
...
function groupBoxesByArticle(
  boxRows: BoxRow[],
  imageRows: ImageRow[],
): Map<number, ArticleBoxes> {
    ...
    case "slider": {
      ...
      if (images.length >= 4) {
        ...
        if (imageRefs.length >= 4) {
          ...
          console.log(
            `   ⚠️  Gallery/slider has less than 4 images after upload, skipping (BoxID: ${box.BoxID})`,
          );
        }
      } else {
        console.log(
          `   ⚠️  Gallery/slider has less than 4 images in CSV, skipping (BoxID: ${box.BoxID}, count: ${images.length})`,
        );
      }
      break;
    }
...
async function migrateArticleContent(options: CliOptions): Promise<void> {
  ...
  console.log(
    `   Found ${boxRows.length} boxes and ${imageRows.length} gallery images`,
  );
  ...
  if (options.articleId) {
    articlesToProcess = articlesToProcess.filter(
      (a) => a.articlePageId === options.articleId,
    );
  }
  if (options.limit) {
  ...
  for (const article of articlesToProcess) {
    ...
    try {
      ...
      for (const box of article.boxes) {
        const boxBlocks = await processBox(
          box,
          article.imagesByBoxId,
          client,
          assetCache,
          options.dryRun,
        );
      ...
      if (options.dryRun) {
        ...
        console.log(
          `   🧪 DRY RUN - Would update document with slug: ${expectedSlug}`,
        );
        if (options.verbose) {
        ...
        if (!targetDocId) {
          ...
          if (!existingDoc) {
            console.log(
              `   ⚠️  Document not found in Sanity for slug: ${expectedSlug}, skipping`,
            );
            continue;
          }
