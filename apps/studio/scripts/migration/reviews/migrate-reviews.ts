...
function replaceSiteTreeLinks(input?: string | null): string {
  if (!input) return '';
  return input.replace(SITE_TREE_LINK_REGEX, (_match, id) => `/recenzje/${id}/`);
}
...
function serializeNode(node: any, activeMarks: string[] = []): SerializedResult {
...
function ensureReviewSlug(rawSlug: string | undefined, fallbackTitle: string): string | null {
...
async function fetchAuthorMap(client: SanityClient): Promise<Map<string, string>> {
  const docs = await client.fetch<Array<{ _id: string; name: string }>>(
    '*[_type == "reviewAuthor"]{_id, name}'
  );
  const map = new Map<string, string>();
  docs.forEach((doc) => {
    const slug = slugify(cleanString(doc.name), { lower: true, strict: true, trim: true });
...
function findAuthorReference(
  ...
  const slug = slugify(cleanString(authorName), { lower: true, strict: true, trim: true });
  const ref = authorMap.get(slug);
  if (!ref) {
    console.warn(`   ⚠️  Author "${authorName}" not found in Sanity (expected _id review-author-${slug})`);
    return undefined;
  }
...
function parseLegacySections(raw?: string): LegacySection[] {
  ...
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed
        .map((section) => ({
          sort: typeof section.sort === 'number' ? section.sort : parseInt(section.sort ?? '0', 10) || 0,
          ...
          content:
            typeof section.content === 'string'
              ...
              : '',
          publish: typeof section.publish === 'number' ? section.publish : parseInt(section.publish ?? '1', 10) || 0,
        }))
...
async function buildContentNodesFromSections(
  ...
  for (const section of sortedSections) {
    ...
    if (html) {
      const sectionNodes = await buildContentNodesFromHtml(html, client, assetCache, dryRun);
...
function buildDescriptionText(row: ReviewRow, sections: LegacySection[]): string {
  ...
  for (const section of sections) {
    ...
    if (sectionHtml) {
      return stripHtml(sectionHtml).split(/\n+/).filter(Boolean).slice(0, 2).join(' ');
    }
  }
  return '';
}
...
function resolveImageSource(src: string): { url: string; filename: string } | null {
  if (!src) return null;
  ...
  if (!/^https?:\/\//i.test(cleaned)) {
    ...
    cleaned = cleaned.replace(/(https?:\/\/(www\.)?audiofast\.pl\/assets\/)/i, LEGACY_ASSETS_BASE_URL);
  }
...
async function buildContentNodesFromHtml(
  ...
  for (const token of tokens) {
    ...
    if (!resolved) {
      console.warn(`   ⚠️  Could not resolve inline image source "${token.src}"`);
      continue;
    }
...
async function buildReviewDocument(
  ...
  const titleSource = cleanString(row.PageTitle) || cleanString(row.MenuTitle) || `Review ${id}`;
  ...
  const contentNodes =
    destinationType === 'page'
      ? sections.length > 0
    ? await buildContentNodesFromSections(sections, client, assetCache, dryRun)
    : await buildContentNodesFromHtml(contentHtml, client, assetCache, dryRun)
      : [];
  ...
  const slug = destinationType === 'page' ? ensureReviewSlug(row.Slug, titleSource) : null;
  ...
  if (!authorRef) {
    console.warn(`   ⚠️  Review ${id} has unknown author "${authorName || row.AuthorName}", skipping`);
    return null;
  }
  ...
  if (destinationType === 'pdf' && pdfFilename) {
    ...
    if (!pdfAssetId) {
      console.warn(`   ⚠️  Review ${id} PDF missing (filename: ${pdfFilename || row.PDFFilename}), skipping`);
      return null;
    }
  }
  ...
  const document: ReviewDocument = {
    ...
    pdfFile:
      destinationType === 'pdf' && pdfAssetId
        ...
        : undefined,
    externalUrl: destinationType === 'external' && externalLink ? externalLink : undefined,
    seo: {
      title: seoTitle,
...
async function migrateReviews(rows: ReviewRow[], options: CliOptions): Promise<void> {
  const titleFilter = options.titleFilter ? options.titleFilter.trim().toLowerCase() : undefined;
  const authorFilter = options.authorFilter ? options.authorFilter.trim().toLowerCase() : undefined;

  const filtered = rows.filter((row) => {
    if (titleFilter) {
      const rowTitle = (cleanString(row.PageTitle) || cleanString(row.MenuTitle)).toLowerCase();
  ...
  for (const row of filtered) {
    const doc = await buildReviewDocument(row, client, authorMap, assetCache, options.dryRun);
    if (doc) {
...
async function main() {
  ...
  if (options.authorFilter) console.log(`Author Filter: ${options.authorFilter}`);
