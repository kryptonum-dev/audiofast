...
function readProductFlagsByPriceKey(
  ...
  for (
    ...
    const existing =
      flagsByPriceKey.get(priceKey) || makeB2CFlags(false, false);
    flagsByPriceKey.set(
      priceKey,
      makeB2CFlags(
        existing.is_sellable_online ||
          parseTakFlag(
            cellStr(row, PRICING_SYNC_CONFIG.PRODUCT_SELLABLE_ONLINE_COL),
          ),
        existing.is_returnable ||
          parseTakFlag(
            cellStr(row, PRICING_SYNC_CONFIG.PRODUCT_RETURNABLE_COL),
          ),
      ),
    );
  }
...
function getProductFlagStats(variants: Map<string, Variant>): ProductFlagStats {
  ...
  for (const variant of variants.values()) {
    const existing =
      byPriceKey.get(variant.price_key) || makeB2CFlags(false, false);
    byPriceKey.set(
      variant.price_key,
      makeB2CFlags(
...
function readCpo(workbook: ExcelScript.Workbook): CpoProduct[] {
  ...
  for (let i = PRICING_SYNC_CONFIG.DATA_START_ROW_CPO; i < data.length; i++) {
    const row = data[i];
...
function readProdukty(workbook: ExcelScript.Workbook): Map<string, Variant> {
  const sheet = workbook.getWorksheet(PRICING_SYNC_CONFIG.SHEET_PRODUKTY);
...
function readListy(workbook: ExcelScript.Workbook): ListyIndex {
  const sheet = workbook.getWorksheet(PRICING_SYNC_CONFIG.SHEET_LISTY);
  ...
  for (let i = PRICING_SYNC_CONFIG.DATA_START_ROW_LISTY; i < data.length; i++) {
    const row = data[i];
    const product = cellStr(row, 0);
    const model = normalizeModel(cellStr(row, 1));
    const opcja = cellStr(row, 2);
...
function readOpcje(
  ...
  for (let i = PRICING_SYNC_CONFIG.DATA_START_ROW_OPCJE; i < data.length; i++) {
    const row = data[i];
    const product = cellStr(row, 0);
...
async function main(workbook: ExcelScript.Workbook): Promise<void> {
  ...
  try {
    ...
    if (result.supabase?.counts) {
      console.log(
        `Zaktualizowano w Supabase: ${result.supabase.counts.variants} wariantów`,
      );
    }
    if (
    ...
    if (result.sanity) {
      console.log(
        `Sanity ceny/flagi: ${result.sanity.prices || 'brak statusu'}`,
      );
      console.log(
        `Powiązane produkty: ${result.sanity.related_products || 'brak statusu'}`,
      );
      console.log(
        `Warianty z powiązanymi produktami: ${result.sanity.related_products_count || 0}`,
      );
    }
    if (result.cpo) {
      if (result.cpo.error) {
        console.log(`CPO: błąd synchronizacji (${result.cpo.error})`);
      } else {
        console.log(
