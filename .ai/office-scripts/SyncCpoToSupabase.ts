...
async function main(workbook: ExcelScript.Workbook): Promise<void> {
  ...
  try {
    const settingsSheet = workbook.getWorksheet(
      CPO_SYNC_CONFIG.SHEET_USTAWIENIA,
    );
    if (!settingsSheet) {
    ...
    const password = String(
      settingsSheet.getRange(CPO_SYNC_CONFIG.PASSWORD_CELL).getValue() || '',
    )
      .trim()
      .replace(/[^\x20-\x7E]/g, '');
    ...
    const response = await fetch(
      CPO_SYNC_CONFIG.SUPABASE_URL + '/functions/v1/cpo-product-sync',
      {
        method: 'POST',
        headers: {
